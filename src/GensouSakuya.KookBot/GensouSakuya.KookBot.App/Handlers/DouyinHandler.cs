...
{
    [CommandTrigger("douyin")]
    internal class DouyinHandler : BaseDataHandler<ConcurrentDictionary<ulong, ConcurrentDictionary<string, DouyinSubscribeModel>>>
    {
        private DouyinWorker _worker;
        public DouyinHandler(IDataService dataService, IKookService kookService, ILogger logger) : base(dataService,kookService, logger)
        {
            _worker = new DouyinWorker(kookService, logger, GetData);
        }
        ...
        public override async Task NextAsync(EnumKookMessageSource source, string? originMessage, BaseKookUser sendBy, BaseKookChannel sendFrom)
        {
            ...
            if (!ulong.TryParse(commands[2], out var roomId))
            {
                return;
            }
            ...
            if (first == "subscribe")
            {
                var sub = subscribers.GetOrAdd(roomId, new ConcurrentDictionary<string, DouyinSubscribeModel>());
                if (sub.ContainsKey(sbm.ToString()))
                {
                    await SendBack(source, "该直播间已订阅", sendFrom);
                    return;
                }
    ...
    internal class DouyinSubscribeModel
    {
        ...
        {
            return obj is DouyinSubscribeModel model &&
                   Source == model.Source &&
